...
public final class CF1914D {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            out.println(solve(0,0, dp, n, a,b,c));
    ...
    private static long solve(int i, int state, Long dp[][], int n, long a[], long b[], long c[]){
        if(state == 7){
            return 0l;
        }
        if(i==n){
        ...
        long ans = solve(i+1, state, dp, n, a, b, c);
        if((state&1)==0){
            ans = Math.max(ans, a[i]+solve(i+1, state | 1, dp, n, a, b, c));
        }

        if((state&2)==0){
            ans = Math.max(ans, b[i]+solve(i+1, state | 2, dp, n, a, b, c));
        }

        if((state&4)==0){
            ans = Math.max(ans, c[i]+solve(i+1, state | 4, dp, n, a, b, c));
        }
    ...
    static class Pair{
        ...
        public Pair(int ind, long val, char ch){
            this.val =val;
            this.ind = ind;
            this.ch = ch;
        }
