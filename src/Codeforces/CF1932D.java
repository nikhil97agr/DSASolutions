...
public final class CF1932D {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            for(String s : cards){
                map.computeIfAbsent(s.charAt(1), ch -> new ArrayList<>()).add(s);
            }

            if (map.containsKey(trump)) {
                List<String> list = map.get(trump);
                boolean res = true;
                int ind = 0;
                List<String[]> ans = new ArrayList<>();
                for(Map.Entry<Character, List<String>> entry : map.entrySet()){
                    if(entry.getKey()!=trump && entry.getValue().size()%2==1){
                        if(ind < list.size()){
                            ans.add(new String[]{entry.getValue().get(0), list.get(ind)});
                            ind++;
                        }else{
                            res = false;
                            break;
                ...
                if((list.size() - ind)%2==1){
                    res = false;
                }
                if(!res){
                    out.println("IMPOSSIBLE");
                }else{
                    for(;ind< list.size();ind+=2){
                        String s1 = list.get(ind);
                        String s2 = list.get(ind+1);
                        if(s1.compareTo(s2) > 0){
                            ans.add(new String[]{s2, s1});
                        }else{
                            ans.add(new String[]{s1, s2});

                        }
                    }

                    for(Map.Entry<Character, List<String>> entry : map.entrySet()){
                        if(entry.getKey()!= trump){
                            int i=0;
                            if(entry.getValue().size()%2==1){
                                i++;
                            }
                            for(;i<entry.getValue().size();i+=2){
                                String s1 = entry
                                        .getValue().get(i);
                                String s2 = entry.getValue().get(i+1);
                                if(s1.compareTo(s2)>0){
                                    ans.add(new String[]{s2, s1});
