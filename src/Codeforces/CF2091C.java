...
public final class CF2091C {
    ...
    private static void generate(int n, int arr[], int i, int state){
        if(i==n){
            Set<Integer> set = new HashSet<>();
            for(int j=0;j<n;j++){
                int pos = j+1;
                int steps = arr[j] == pos ? 1 : (arr[j] > pos ? (arr[j] - pos + 1) : (n-pos + 1) + arr[j]);
                set.add(steps);
            }
