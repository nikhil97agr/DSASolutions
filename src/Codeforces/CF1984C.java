...
public final class CF1984C {
    ...
    private static void solve(PrintWriter out){
        ...
        for(int i=1;i<=n;i++){
            dp[i][0] = min(dp[i-1][0] + arr[i-1], dp[i-1][1] + arr[i-1]);
            dp[i][1] = max(abs(dp[i-1][0] + arr[i-1]), abs(dp[i-1][1] + arr[i-1]));
        }

        out.println(max(dp[n][0], dp[n][1]));
    }
