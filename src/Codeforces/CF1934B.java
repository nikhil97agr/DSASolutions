...
public final class CF1934B {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            long c[] = new long[]{15,10,6,3,1};

            long ans = solve(c, n, 0, c.length);
            out.println(ans);
        }
    ...
    private static long solve(long c[], long n, int ind, int m){
        if(n==0) return 0;
        if(ind == m-1){
            return n;
        }
        ...
        if(n > c[ind]){
            return Math.min(solve(c, n%c[ind] + c[ind], ind+1, m) + n/c[ind] - 1, solve(c, n%c[ind], ind+1, m) + n/c[ind]);
        }else{
            return solve(c, n, ind+1, m);
        }
