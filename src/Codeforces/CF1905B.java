    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            for(int i=0;i<n-1;i++){
                ...
                degree[u]++;
                degree[v]++;
                map.computeIfAbsent(u, ver -> new ArrayList<>()).add(v);
                map.computeIfAbsent(v, ver -> new ArrayList<>()).add(u);
            }
            Map<Integer, Integer> mapping = new HashMap<>();
            int ans = 0;
            for(int i=1;i<=n;i++){
                if(degree[i]==1){
                    addToMap(map.get(i).get(0), mapping);
                }
            }

            List<Integer> values = new ArrayList<>(mapping.values());
