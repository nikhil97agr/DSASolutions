...
public final class CF2227D {
    ...
    private static void solve(PrintWriter out) {
        ...
        for (int i = 0; i < 2 * n; i++) {
            if (arr[i] == 0) {
                list.add(i);
                fill(set, n);
                for (int j = i - 1, k = i + 1; j >= 0 && k < 2 * n && arr[j] == arr[k]; j--, k++) {
                    set.remove(arr[j]);
                }

                ans = Math.max(ans, set.first());
            }
        }
        fill(set, n);
        int i;
        int j;
        for (i = list.getFirst() + 1, j = list.getLast() - 1; i <= j && arr[i] == arr[j]; i++, j--) {
            set.remove(arr[i]);
        }
        ...
        for (i = list.getFirst() - 1, j = list.getLast() + 1; i >= 0 && j < 2 * n && arr[i] == arr[j]; i--, j++) {
            set.remove(arr[i]);
        }
    ...
    private static void fill(TreeSet<Integer> set, int n) {
