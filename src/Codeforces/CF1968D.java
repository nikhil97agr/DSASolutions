...
public final class CF1968D {
    private final static long mod = (long)1e9+7;
    private final static FastReader reader = new FastReader();
    private final static String BODYA = "Bodya";
    private final static String SASHA = "Sasha";
    ...
    private static void solve(PrintWriter out){
        ...
        int b = read();
        int s = read();
        int p[] = intArray(n, true);
        long a[] = longArray(n, true);
        long sb = solve(new boolean[n+1], p, a, b, k);
        long ss = solve(new boolean[n+1], p, a, s, k);

        if(sb > ss) out.println(BODYA);
        else if(ss > sb) out.println(SASHA);
        else out.println(DRAW);
    }

    private static long solve(boolean vis[], int p[], long arr[], int i, int k){
        if(vis[i] || k==0){
            return 0L;
        }
        vis[i] = true;
        return max(arr[i]*k, arr[i] + solve(vis, p, arr, p[i], k-1));
