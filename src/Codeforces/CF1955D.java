...
public final class CF1955D {
    ...
    private static void solve(PrintWriter out){
        ...
        int cnt = 0;
        for(int i=0;i<m;i++){
            addToMap(a[i], traversal);
            if(map.getOrDefault(a[i], 0) >= traversal.get(a[i])){
                cnt++;
            }
        }
        int start = 0;
        ...
        while(end < n){
            ...
            removeFromMap(a[start], traversal);
            if(map.containsKey(a[start]) && map.get(a[start]) > traversal.getOrDefault(a[start], 0)){
                cnt--;
            }
            start++;
            addToMap(a[end], traversal);
            if(map.getOrDefault(a[end], 0) >= traversal.get(a[end])){
                cnt++;
            }
            end++;
        }
