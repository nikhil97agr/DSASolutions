...
class CCStart112C {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            ans[0] = set.firstKey();
            removeFromMap(ans[0], set);
            int cnt = 1;
            int ind = 1;
            long sum = ans[0];
            while(!set.isEmpty()){
                Long next = set.higherKey(sum);
                if(next==null) break;
                sum += next;
                removeFromMap(next, set);
                ans[ind++] = next;
                cnt++;
            }
            ...
            out.println(n-cnt);
            while(!set.isEmpty()){
                ans[ind++] = set.firstKey();
                removeFromMap(set.firstKey(), set);
            }
            for(long x : ans){
                out.print(x+" ");
            }
    ...
    private static int solve(long arr[], long sum, int cnt, int i, int n, int ans[], boolean visited[]){
        if(i==n){
        ...
        if(sum >= arr[i])
        {
            return Math.min(solve(arr, sum + arr[i], cnt, i+1, n, ans, visited), solve(arr, sum, cnt, i+1, n, ans, visited));
        }
        else{
            return Math.min(solve(arr, sum + arr[i], cnt-1, i+1, n, ans, visited) , solve(arr, sum, cnt, i+1, n, ans, visited));
        }
    }
